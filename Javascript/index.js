...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateResumePage() {
        resumeImage.src = resumePages[currentPage];
        pageInfo.textContent = `Page ${currentPage + 1} / ${resumePages.length}`;

        prevPageBtn.disabled = currentPage === 0;
        nextPageBtn.disabled = currentPage === resumePages.length - 1;
    }
    ...
    nextPageBtn.addEventListener("click", () => {
        if (currentPage < resumePages.length - 1) {
            currentPage++;
            updateResumePage();
        }
    });
    ...
    document.addEventListener("keydown", (e) => {
        ...
        if (e.key === "ArrowRight" && currentPage < resumePages.length - 1) {
            currentPage++;
            updateResumePage();
        }
    ...
    zoomContainer.addEventListener("wheel", (e) => {
        ...
        scale = Math.min(Math.max(1, scale), 3); // limit 1x → 3x
    ...
    function resetZoom() {
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            const pdfBtn = document.getElementById("certPdfBtn");

            const close1 = document.getElementById("closeCertModal");
            const close2 = document.getElementById("closeCertModal2");
            ...
            modal.addEventListener("click", (e) => {
                if (e.target.classList.contains("cert-backdrop")) {
                    closeModal();
                }
            });
        });
    ...
    const modal = document.getElementById("journeyReview");
    const content = modal.querySelector(".journey-content");
    ...
    document.querySelectorAll(".review-btn").forEach(btn => {
        btn.addEventListener("click", function () {
            ...
            content.classList.remove("from-left", "from-right");
            if (item.classList.contains("left")) {
                content.classList.add("from-left");
            } else {
                content.classList.add("from-right");
            }

            // fill content
            document.getElementById("reviewTitle").textContent = card.dataset.title;
            document.getElementById("reviewDate").textContent = card.dataset.date;
            document.getElementById("reviewDesc").textContent = card.dataset.desc;

            // bullets
            const points = card.dataset.points.split("|");
            const ul = document.getElementById("reviewPoints");
            ul.innerHTML = "";
            points.forEach(p => {
    ...
    // close
    document.querySelector(".close-review").onclick = () => {
        modal.classList.remove("active");
    };
